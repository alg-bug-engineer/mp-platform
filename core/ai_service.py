...
- 确保全文是一篇连贯、高质量、排版优雅（善用加粗和引用）的微信公众号深度专栏文章。"""
    user = (
        ...
        + "\n\n**【最后强调（不遵守则判定为不合格）】**：\n"
        + "1. 必须生成一个全新的、有吸引力的标题！不能使用原素材标题！\n"
        + "2. 绝对不允许出现任何 [建议配图] 或类似占位符！\n"
        + "3. 绝对不允许使用 1. 2. 3. 这种列表式的AI风格，必须写成段落连贯的深度专栏散文！\n"
        + "4. 文章要有深度和干货，篇幅要长，分析要透彻！\n\n"
        + anti_ai_rules
        + "\n**现在，请直接开始创作你的深度专栏推文（以 `# 新标题` 开头）：**\n"
    )
    return system, user
...
def call_openai_compatible(profile: AIProfile, system_prompt: str, user_prompt: str) -> str:
    ...
    try:
        ...
        if resp.status_code >= 400:
            ...
            # 部分模型（如 o1/o3）只允许 temperature=1，自动回退重试
            if "only 1 is allowed" in err_text and "temperature" in err_text:
